---
title: 运行时子系统
description: packages/opencode/src 五个核心子系统的关键文件与入口导航。
---
这章把 `packages/opencode/src` 下五个核心子系统的关键文件与入口列出来，作为读代码的导航图。Session 的**运行时内核**在 core 包，深度拆解见 [Session V2 深度](./session-v2-deep-dive)；本章聚焦 opencode 包的业务侧。

> 提醒：opencode 包是"**跑什么**"（业务编排、工具实现、provider 注册），core 包是"**怎么跑一次会话**"（admission/execution/runner）。见 [架构分层](./architecture)。
...
详见 [工程基建](./tooling#配置系统srcconfig)。核心：每个模块顶部 `export * as ConfigX from "./x"`。Catalog（模型元数据）在 core 包的 `catalog.ts` / `models-dev.ts`。

## storage/ — 存储与 `#db`

详见 [工程基建](./tooling#存储与-db)。`#db` 条件导入切换 Bun/Node 适配；`storage.ts` 是 JSON 文件存储 + migration；`schema.ts` re-export 所有 Drizzle 表（snake_case）。
...
每一步的 file:line 见 [Session V2 深度](./session-v2-deep-dive)。

## 进一步阅读

- 内核机制：[Session V2 深度](./session-v2-deep-dive)
- 分层：[架构分层](./architecture)
- 术语：ToolRegistry / Model Tool Output / Managed Tool Output File → [术语表](./glossary)
