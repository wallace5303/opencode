...
Session V2 的运行时规则（admission/execution 分离、每 turn 一次 `llm.stream`、drain 进程本地、`steer`/`queue` 词汇等）见 `AGENTS.md` 的 "V2 Session Core" 段，术语见 [术语表](./glossary)。深度拆解留待 `04-session-v2-deep-dive`（待写）。
