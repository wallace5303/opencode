---
title: Session V2 深度
description: opencode 最核心的 Session V2 内核：admission/execution 分离、runner、Context Epoch。
sidebarTitle: Session V2 深度
---
这是 opencode 最核心、也最容易读错的部分。所有路径基于 `packages/core/src`——**Session V2 的内核在 core 包，不在 opencode 包**（opencode 侧只有业务编排与 Layer 组装）。

> 先读 [术语表](./glossary) 的"会话与上下文""提示与执行"两节，把 Admitted Prompt / Provider Turn / Session Drain / Context Epoch 这几个词记住。
...
- 术语：[术语表](./glossary)
- 分层为何如此：[架构分层](./architecture)
- 各子系统业务侧：[运行时子系统](./runtime-subsystems)
