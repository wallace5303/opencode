---
title: 多端（TUI / Web / Desktop / SDK）
description: TUI / Web / Desktop / SDK 多端形态与共享 UI 组件。
sidebarTitle: 多端
---
...
所有远程表面都走同一份 HttpApi 契约（`packages/protocol/src/api.ts` 的 `makeDefaultApi`），server 实现，客户端生成。详见 [客户端与 UI](../clients-and-ui)。
...
详见 [客户端与 UI](../clients-and-ui#分层边界再强调)。

## 相关文档

- 客户端与 SDK 全景：[客户端与 UI](../clients-and-ui)
- CLI 命令：[cli-commands](./cli-commands)
- server 实现：[运行时子系统](../runtime-subsystems#server--http-服务器与-httpapi)
