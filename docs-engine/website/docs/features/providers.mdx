---
title: Providers（模型供应方）
description: opencode 通过统一 Provider 抽象对接多家模型供应方。
---
...
详见 [CLI 命令](./cli-commands#provider-与模型)。
...
- 运行时侧文件清单：[运行时子系统](../runtime-subsystems#provider--llm-provider)
- Catalog 与配置：[config](./config)
- 术语：Model Request Options / Generation Controls → [术语表](../glossary)
