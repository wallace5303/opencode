---
title: CLI 命令
description: opencode CLI 子命令分组：交互/非交互运行、会话、agent、serve 等。
---
...
文件：`cli/cmd/agent.ts:254`。Skill 通过配置发现，无独立 CLI 子命令，见 [agents-and-skills](./agents-and-skills)。

## MCP

`opencode mcp` 管理 MCP 服务：`add` / `list` / `auth` / `logout` / `debug`。文件：`cli/cmd/mcp.ts:95`。详见 [agents-and-skills](./agents-and-skills#mcp)。
