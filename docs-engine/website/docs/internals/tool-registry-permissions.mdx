...
description: 工具从定义到被模型调用、受权限约束、输出被截断的全链路。
sidebarTitle: 工具注册与权限
---
...
- 工具在 runner 中的调用点：[llm-stream-loop](./llm-stream-loop#10-工具调用结算)
- 工具功能清单：[tools](../features/tools)
- Agent 与子 agent 权限：[agents-and-skills](../features/agents-and-skills#子-agent-权限)
- 术语：Model Tool Output / Managed Tool Output File / PTY Environment → [术语表](../glossary)
