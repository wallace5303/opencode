...
description: 拆解 runTurnAttempt——SessionRunner 内层 continuation 循环每一步。
sidebarTitle: Provider Turn 链路
---
...
详见 [system-context-algebra](./system-context-algebra)。

### (3) Promote（safe boundary）

见 [session-lifecycle](./session-lifecycle#阶段-5promote提升safe-boundary)。`cutoff = EventV2.latestSequence`，只提升已投影的 input。`promoted>0` → `currentStep=1` 重置配额。

### (4) 模型解析

`models.resolve(session)`（`:194`）→ core 的 `Catalog` 服务查 provider+model → `fromCatalogModel()` 转 AI SDK `LanguageModel`。详见 [运行时子系统](../runtime-subsystems#provider--llm-provider)。
...
工具结果作为 **Model Tool Output**（有界投影）进 Session History。超限的全文写到 **Managed Tool Output File**。详见 [tool-registry-permissions](./tool-registry-permissions)。
...
详见 [sessions](../features/sessions#compaction压缩)。
...
- 上一层时序：[session-lifecycle](./session-lifecycle)
- 工具结算细节：[tool-registry-permissions](./tool-registry-permissions)
- System Context baseline：[system-context-algebra](./system-context-algebra)
