---
title: 会话生命周期
description: 一条用户消息从输入到落地为持久化历史的完整路径。
---
...
详见下一次 provider turn 的拆解 → [llm-stream-loop](./llm-stream-loop)。
...
- 一次 provider turn 内部：[llm-stream-loop](./llm-stream-loop)
- System Context 在 safe boundary 的合并：[system-context-algebra](./system-context-algebra)
- 完整术语：[术语表](../glossary)
