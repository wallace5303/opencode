...
> ⚠️ 注意：**Session V2 的运行时内核不在 `packages/opencode/src/session`，而在 `packages/core/src/session*`**。opencode 包的 `session/` 是业务编排层，调用 core 包的内核。这是初学者最容易踩的坑，详见 [架构分层](./architecture.md)。
...
进入 [架构分层](./architecture.md)，看清楚包之间到底怎么连。
