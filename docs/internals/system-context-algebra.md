...
> 术语严格按 `CONTEXT.md`：用 **System Context**，不用 "system prompt"。详见 [术语表](../glossary.md#会话与上下文)。
...
- safe boundary 与 promote：[session-lifecycle](./session-lifecycle.md#阶段-5promote提升safe-boundary)
- runner 里调 prepare 的位置：[llm-stream-loop](./llm-stream-loop.md#2-system-context-baseline)
- 完整术语：[术语表](../glossary.md#会话与上下文)
