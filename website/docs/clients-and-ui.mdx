---
title: 客户端与 UI
description: HTTP 契约、生成客户端、组合式 SDK 与 SolidJS 多 UI 表面串联。
---
...
> 改了契约 → 跑 `packages/client` 的 `bun run generate` → 生成物更新 → 所有客户端自动跟上。这是整个多端体系的根基，详见 [架构分层](./architecture) 的"HttpApi 链路"。
...
<Tabs>
  ...
  <Tab title="Embedded OpenCode（sdk-next）">
...
- 契约与生成链路：[架构分层](./architecture#三条核心链路)
- server 端 HttpApi 实现：[运行时子系统](./runtime-subsystems) 的 server 段
- 术语：OpenCode Client / Embedded OpenCode / SDK Contract IR / Page → [术语表](./glossary)
