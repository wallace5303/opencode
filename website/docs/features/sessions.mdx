---
title: 会话管理
description: 会话业务功能：创建/恢复、fork、revert、compaction、分享、worktree。
---
会话（Session）是 opencode 的核心持久化单位。这章讲业务侧的功能：创建/恢复、fork、revert、compaction、分享、worktree。运行时内核见 [Session V2 深度](../session-v2-deep-dive)。
...
底层 `SessionV2.prompt`（`packages/core/src/session.ts:360`）admit 一条 `session_input` 后唤醒执行，详见 [Session V2 深度](../session-v2-deep-dive)。
...
2. 用内置 **compaction agent**（见 [agents-and-skills](./agents-and-skills)）对历史生成摘要
...
详见 [CLI 命令](./cli-commands#会话管理)。

## 相关文档

- 运行时内核：[Session V2 深度](../session-v2-deep-dive)
- Agent 与 compaction agent：[agents-and-skills](./agents-and-skills)
- 术语：Admitted Prompt / Provider Turn / Session Drain → [术语表](../glossary)
