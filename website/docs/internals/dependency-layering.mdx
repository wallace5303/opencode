---
title: 分层约束如何落地
description: 分层约束如何被代码保证：package.json、import 防线、构建期检查。
sidebarTitle: 分层约束落地
---
[架构分层](../architecture) 给了铁律：`Schema → Core/Protocol → Server`，Client 只依赖 Schema/Protocol，`sdk-next` 组合三者。这章讲它**怎么被代码保证**，而不只是靠自觉。
...
- 分层全景与各包依赖：[架构分层](../architecture)
- 生成链路细节：[httpapi-codegen](./httpapi-codegen)
- 嵌入式用法：[embedded-opencode](./embedded-opencode)
